...
# TEMPORARY STATE FIX
# replace with your real state column if notebook created one
...
st.header("Median Lead Time by State with Rural Population Percentage")

# PLACEHOLDER VALUES
rural_population = {
    state: i * 3 + 5
    for i, state in enumerate(filtered_states.index)
}
...
scatter_df = pd.DataFrame({
    ...
    "rural_population_pct": [
        rural_population[s]
        for s in filtered_states.index
    ]
})
...
ax4.scatter(
    scatter_df["rural_population_pct"],
    scatter_df["median_lead_time"]
)

for i, row in scatter_df.iterrows():
    ax4.text(
        row["rural_population_pct"],
        row["median_lead_time"],
        row["state"]
    )

ax4.set_xlabel("Rural Population Percentage")
ax4.set_ylabel("Median Lead Time (minutes)")
ax4.set_title(
    "Median Evacuation Lead Time vs Rural Population %"
)
